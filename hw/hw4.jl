...
function recovery_time(p)
	...
	while true
		τ += 1
		if bernoulli(p::Number) break end
	end
...
Patient is guaranteed to recover in one day
"""
...
function frequencies(value_vec)
	freq = Dict()
	n = length(value_vec)
	for v in value_vec
		if haskey(freq, v) freq[v] += 1 else freq[v] = 1 end
	end
	map!(v -> v/n, values(freq))
	return freq
end
...
# ╔═╡ c9fa6640-16be-11eb-3d72-91829cca8f8c
function sin_cos_plot()
    T = -1.0:0.01:1.0
...
# ╔═╡ cd0498fe-16be-11eb-19f5-7b98615cfa1b
sin_cos_plot()
...
function frequencies_plot_with_mean(data)
	base = bar(frequencies(data))
	vline!(base, [sum(data)/length(data)], label="maximum")
	return base
end
...
begin
  ...
  result = frequencies_plot_with_mean(interactive_experiment)
  x = 1:60
  λ = sum(interactive_experiment)/N_interactive
  plot!(result, x, exp.(-x./λ)./λ)
  result
end
...
md"See solution added to previous block -- exponential"
