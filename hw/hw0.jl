...
ex_1_1 = md"""
Suppose $a,x > 0$. Either $a \leq \sqrt{x}$, or $a \geq \sqrt{x}$. The following equivalence holds

$$a \leq \sqrt{x} \iff \sqrt{x}a \leq x \iff \sqrt{x} \leq \frac{x}{a}.$$
So, by the above, if $a \leq \sqrt{x}$ then  $a \leq \sqrt{x} \leq \frac{x}{a}$. The dual result holds for $a \geq \sqrt{x}.$
"""
...
#	a₂ = ( x / a + a ) / 2
#	if ( abs(a₂ - a) < error_margin ) return a₂ end
#	return newton_sqrt(x, error_margin, a₂)
#end

function newton_sqrt(x, error_margin=0.01, a=x / 2) # a=x/2 is the default value of `a`
	a₂ = ((x/a)+a)/2
	if (abs(a₂ - a) < error_margin) 
		return a₂ # this is wrong, write your code here!
	else (return newton_sqrt(x, error_margin, a₂))
	end
end
...
# ╔═╡ b6b44430-ee90-11ea-10b0-ffd9883e4619
